...
export class SolicitudesIngresoComponent implements OnInit {
  ...
  obtenerSolicitudesPendientes() {
    this.ingresoService
      .getIngresos(undefined, undefined, 'Pendiente')
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          if (error.status === 404) {
            ...
            this.errorMensaje =
              'Error al cargar las solicitudes de ingreso. Por favor, inténtalo de nuevo.';
          }
          this.solicitudesPendientes = [];
        },
      });
  ...
  darDeAlta() {
    ...
    this.historialAltaService.addHistorialAlta(historialAlta).subscribe({
      next: (response) => {
  ...
  obtenerCamasDisponibles(tipoCama: string) {
    ...
    this.asignacionService
      .getAsignaciones(undefined, undefined, new Date(), null, undefined)
      .subscribe({
        next: (asignaciones) => {
          ...
          this.camaService.getCamas().subscribe({
            next: (camas: Cama[]) => {
              ...
              const camasOcupadas = asignaciones.map(
                (asignacion) => asignacion.IdCama
              );
              this.camasDisponibles = camas.filter(
                (cama) =>
            ...
            error: (error: HttpErrorResponse) => {
              console.error(
                'Error al obtener las camas disponibles:',
                error.message
              );
            },
          });
        ...
        error: (error: HttpErrorResponse) => {
          console.error(
            'Error al obtener las asignaciones activas:',
            error.message
          );
        },
      });
