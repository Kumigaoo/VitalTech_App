...
export class IngresosAdministrativoComponent implements OnInit {
  ...
  obtenerPacientesIngresados() {
    this.ingresoService
      .getIngresos(undefined, undefined, 'Ingresado')
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMensaje =
            'Error al cargar los ingresos. Por favor, inténtalo de nuevo.';
          this.ingresos = [];
        },
      });
  }
    ...
    this.historialAltaService.addHistorialAlta(altaData).subscribe({
      next: () => {
        ...
        this.asignacionService
          .getAsignaciones(this.ingresoSeleccionado!.IdPaciente)
          .subscribe({
            next: (asignaciones: Asignacion[]) => {
              if (asignaciones.length > 0) {
                ...
                this.asignacionService.updateAsignacion(asignacion).subscribe({
                  next: () => {
                    window.alert(
                      'Paciente dado de alta correctamente y asignación actualizada.'
                    );
                ...
                window.alert(
                  'No se encontró ninguna asignación para este paciente.'
                );
              }
            },
            error: () => {
